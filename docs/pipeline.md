...
3. For each remaining bill, get the full details. Batch these requests.
   Bills that are only introduced (LegiScan status 1, our `proposed`) are dropped here as well. Nothing grades them, so they are not worth the text or the agents; the skip is keyed to the change hash, so a bill that advances is picked up then. Bills saved as proposed before this rule (September 2026) are still in the database.
